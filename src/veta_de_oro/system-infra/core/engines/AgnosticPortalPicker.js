...
class AgnosticPortalPicker {
  ...
  render() {
    ...
    // El Input vive en su contenedor natural
    ...
    // El Portal vive en el Body (Agnostic Rendering)
  ...
  bind() {
    ...
    // Detectar clics fuera para cerrar
    ...
    // Actualizar geometría si la pantalla cambia
  ...
  open() {
      this.isOpen = true;
      this.dom.portal.classList.add('active');
      this.updateGeometry();
      this.refreshPortal();
  }
  ...
  close() {
      this.isOpen = false;
      this.dom.portal.classList.remove('active');
  }
  ...
  destroy() {
      if(this.dom.portal && this.dom.portal.parentNode) {
          this.dom.portal.parentNode.removeChild(this.dom.portal);
      }
  }
}
